...
class LinkedInResearchAgent:
    ...
    def _scrape_techcrunch(self, state: ResearchAgentState) -> ResearchAgentState:
        ...
        print(f"Found {len(unique_articles)} unique articles from TechCrunch")
        
        # Convert list to JSON text
        articles_json = json.dumps(unique_articles, indent=2)
    ...
    def _select_news(self, state: ResearchAgentState) -> ResearchAgentState:
        ...
        print(f"[STEP 3/5] Selecting most relevant news items")
        news_items = state["news_content"]["raw_content"]
        
        messages = select_news_prompt.format(messages=[{
            "role": "human",
            "content": "News Articles:\n%s".format(state["news_content"]["raw_content"])
        }])
        response = self.llm.invoke(messages)
        
        try:
            # Parse the response to get selected indices
            selected_indices = json.loads(response.content.strip())
    ...
    def _plan_posts(self, state: ResearchAgentState) -> ResearchAgentState:
        ...
        print(f"[STEP 4/5] Planning LinkedIn posts for the week")

        selected_news = state["selected_news"]
        
        messages = select_news_prompt.format(messages=[{
            "role": "human",
            "content": "Selected News Articles:\n%s".format(json.dumps(selected_news))
        }])
        response = self.llm.invoke(messages)
        
        try:
            # Parse the response to get the posting plan
            posting_plans = json.loads(response.content.strip())
    ...
    # def _get_previous_topics(self) -> List[str]:
    #     """Get previously posted topics from the SQLite database."""
    #     try:
    #         conn = sqlite3.connect(self.db_path)
    #         cursor = conn.cursor()
            
    #         # Get all keywords from topics table
    #         cursor.execute("SELECT keyword FROM topics")
    ...
    def _create_workflow(self) -> StateGraph:
        ...
        workflow.add_edge("plan_posts", "save_to_database")
...
def create_linkedin_research_agent(stream: bool = False, topics: List[str] = None) -> StateGraph:
    """Create and return a LinkedIn research agent instance."""
    agent = LinkedInResearchAgent(stream=stream, topics=topics)
